...
def parse_args():
    ...
    parser.add_argument('--exp-info', default='')
...
if __name__ == '__main__':
    ...
    exp_name = f'{config_file_name}-{cfg.max_lr}-{cfg.clip_num}x{cfg.crop_scale[0]}x{cfg.crop_scale[1]}-data_{cfg.data_version}-fold_{cfg.fold}'
    if args.exp_info != '':
        exp_name += f'-{args.exp_info}'

    cfg.work_dir = os.path.join(cfg.work_dir, datetime.datetime.now().strftime("%Y-%m-%d"), exp_name, datetime.datetime.now().strftime("%H:%M:%S"))
